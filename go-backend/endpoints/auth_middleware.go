...
func (shared *SharedContext) ContributorRequired(c *gin.Context) {
	...
	if err != nil {
		c.AbortWithStatusJSON(http.StatusBadRequest, gin.H{"ok": false, "msg": "invalid cookie"})
		return
	}
	...
	if user.Permission < models.ContributorPermission {
		c.AbortWithStatusJSON(http.StatusBadRequest, gin.H{"ok": false, "msg": "unauthorized"})
		return
	}
